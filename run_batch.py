...
#arguments_inputdir = '/mnt/lustre/share/dingmingyu/cityscapes/'
#arguments_inputlist = 'a_train.txt'
#arguments_outdir = './output/'
arguments_inputdir = ''
arguments_inputlist = 'flow_list.txt'
arguments_outdir = './output_cityscapes/'

for strOption, strArgument in getopt.getopt(sys.argv[1:], '', [ strParameter[2:] + '=' for strParameter in sys.argv[1::2] ])[0]:
	if strOption == '--model':
		arguments_strModel = strArgument # which model to use

	elif strOption == '--inputdir':
		arguments_inputdir = strArgument # path to the first frame

	elif strOption == '--inputlist':
		arguments_inputlist = strArgument # path to the second frame

	elif strOption == '--outdir':
		arguments_outdir = strArgument # path to where the output should be stored
...
if __name__ == '__main__':
    for index, line in enumerate(open(arguments_inputlist).readlines()):
        print(index)
        arguments_strFirst, arguments_strSecond = line.strip().split()
        tensorFirst = torch.FloatTensor(numpy.array(PIL.Image.open(arguments_inputdir+arguments_strFirst))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
        tensorSecond = torch.FloatTensor(numpy.array(PIL.Image.open(arguments_inputdir+arguments_strSecond))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))

        tensorOutput = estimate(tensorFirst, tensorSecond)

        objectOutput = open(arguments_outdir + '%d.flo' % index, 'wb')
